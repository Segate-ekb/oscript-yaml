...
// Хранилище текущих TAG директив для префиксов
Перем ТекущиеТАГДирективы;

// Менеджер якорей и алиасов
Перем МенеджерЯкорей;
...
// Проверяет, является ли строка строкой в кавычках (одинарных или двойных)
//
// Параметры:
...
//
Функция ЯвляетсяСтрокойВКавычках(Строка)
    ...
    ПерваяПозиция = 1;
    Пока ПерваяПозиция <= СтрДлина(Строка) И (Сред(Строка, ПерваяПозиция, 1) = " " Или Сред(Строка, ПерваяПозиция, 1) = Символы.Таб) Цикл
        ПерваяПозиция = ПерваяПозиция + 1;
    КонецЦикла;
    
    Если ПерваяПозиция > СтрДлина(Строка) Тогда
        Возврат Ложь;
    КонецЕсли;
    
    ПервыйСимвол = Сред(Строка, ПерваяПозиция, 1);
    
    // Проверяем, начинается ли строка с кавычек
...
Функция ОбработатьОбычнуюСтроку(Строка, Документ)
    ...
    Лог.Отладка("ОбработатьОбычнуюСтроку: строка='" + Строка + "', Документ=" + ?(Документ = Неопределено, "Неопределено", "Массив[" + Документ.Количество() + "]"));
    ...
    Если СтрНачинаетсяС(СтрокаБезПробелов, "%TAG") Тогда
        ...
        Если Документ <> Неопределено И Документ.Количество() > 0 Тогда
            ВызватьИсключение "Директива %TAG не может появляться в середине документа без завершения документа маркером '...'";
        КонецЕсли;
...
Функция НайтиТипДанныхПоСтроке(Знач Строка)
    ...
    Если СтрНачинаетсяС(ОчищеннаяСтрока, "%YAML") ИЛИ СтрНачинаетсяС(ОчищеннаяСтрока, "%TAG") Тогда
    ...
    Если СтрНачинаетсяС(ОчищеннаяСтрока, "!") Тогда
        ...
        ЭтоVerbatimТег = СтрНачинаетсяС(ОчищеннаяСтрока, "!<");
        
        Если НЕ ЭтоVerbatimТег Тогда
        ...
        Если НайденнаяПозиция > 0 И МинимальнаяНайденнаяПозиция > НайденнаяПозиция Тогда
            ...
            Если Символ = "!" Тогда
                ...
                Если НЕ СтрНачинаетсяС(ОчищеннаяСтрока, "!") Тогда
                    Продолжить; // Пропускаем ! если он не в начале значения
                КонецЕсли;
            КонецЕсли;
            ...
            Если (Символ = ">" ИЛИ Символ = "|") И ПозицияВнутриТега(Строка, НайденнаяПозиция) Тогда
                Продолжить; // Пропускаем символы > и | если они внутри тегов
            КонецЕсли;
...
Функция ЭтоЧисло(Знач Строка)
    ...
    Если СтрНачинаетсяС(ВРег(Строка), "0X") И СтрДлина(Строка) > 2 Тогда
...
Функция ПреобразоватьИзШестнадцатеричной(ШестнадцатеричнаяСтрока)
    ...
    Пока Позиция >= 1 Цикл
        ...
        ИначеЕсли Символ >= "A" И Символ <= "F" Тогда
            ЗначениеЦифры = КодСимвола(Символ) - КодСимвола("A") + 10;
        Иначе
            // Недопустимый символ
            Возврат 0;
        КонецЕсли;
...
Функция ПарсерОбъектов(Строки)
    ...
    Для Каждого Объект Из МассивОбъектов Цикл
        ...
        Иначе
            ...
            Если СтрНайти(Объект.ИмяОбъекта, "!") > 0 Тогда
                МассивКлюча = Новый Массив;
                МассивКлюча.Добавить(Объект.ИмяОбъекта);
                ОбработанныйКлюч = ДанныеИзМассиваСтрок(МассивКлюча);
                
                // Если ключ был обработан как примитивное значение, используем его
                Если ТипЗнч(ОбработанныйКлюч) = Тип("Строка") ИЛИ ТипЗнч(ОбработанныйКлюч) = Тип("Число") ИЛИ ТипЗнч(ОбработанныйКлюч) = Тип("Булево") Тогда
                    ИтоговыйКлюч = Строка(ОбработанныйКлюч);
                Иначе
                    ИтоговыйКлюч = УбратьКавычки(Объект.ИмяОбъекта);
                КонецЕсли;
            Иначе
                ИтоговыйКлюч = УбратьКавычки(Объект.ИмяОбъекта);
            КонецЕсли;
...
Функция ПарсерТегов(Строки)
    ...
    Если СтрНачинаетсяС(СокрЛП(ТегИЗначение.Значение), "&") Тогда
        ...
        СтрокаЗначения = СокрЛП(ТегИЗначение.Значение);
        ...
        Если НЕ ПустаяСтрока(ЗначениеПослеЯкоря) Тогда
            // Используем значение с той же строки
            РезультатПарсинга = ЗначениеПослеЯкоря;
        ...
        МенеджерЯкорей.СохранитьЯкорь(ИмяЯкоря, РезультатПрименения);
    ...
    ИначеЕсли СтрНайти(ТегИЗначение.Значение, ":") > 0 Тогда
        ...
        Возврат Объект;
    ИначеЕсли (СтрНачинаетсяС(СокрЛП(ТегИЗначение.Значение), "{") ИЛИ СтрНачинаетсяС(СокрЛП(ТегИЗначение.Значение), "[")) И Строки.Количество() > 1 Тогда
    ...
    ИначеЕсли ПустаяСтрока(ТегИЗначение.Значение) И Строки.Количество() > 1 Тогда
        ...
        ИначеЕсли ТегИЗначение.Тег = "!!seq" И ТипЗнч(РезультатПарсинга) = Тип("Массив") Тогда
            Возврат РезультатПарсинга;
        ИначеЕсли ТегИЗначение.Тег = "!!set" И ТипЗнч(РезультатПарсинга) = Тип("Соответствие") Тогда
            // Для !!set применяем тег к объекту Соответствие напрямую
            РезультатПрименения = ПрименитьТегКЗначению(ТегИЗначение.Тег, РезультатПарсинга);
            Возврат РезультатПрименения;
        ИначеЕсли ТегИЗначение.Тег = "!!omap" И ТипЗнч(РезультатПарсинга) = Тип("Массив") Тогда
        ...
        Иначе
            ...
            Если ТегИЗначение.Тег = "!!str" И РезультатПарсинга = Неопределено Тогда
                РезультатПрименения = ПрименитьТегКЗначению(ТегИЗначение.Тег, "");
            Иначе
                РезультатПрименения = ПрименитьТегКЗначению(ТегИЗначение.Тег, РезультатПарсинга);
            КонецЕсли;
            Возврат РезультатПрименения;
    ...
    Иначе
        ...
        Если СтрНачинаетсяС(СокрЛП(ТегИЗначение.Значение), """") И НЕ СтрЗаканчиваетсяНа(СокрП(ТегИЗначение.Значение), """") И Строки.Количество() > 1 Тогда
            ...
            РезультатПрименения = ПрименитьТегКЗначению(ТегИЗначение.Тег, ПолнаяСтрока);
        Иначе
...
Функция ОбработатьОбъектСДвоеточием(Строка, ПозицияДвоеточия, ТекущийОтступ)
    ...
    Если НЕ ПустаяСтрока(ОстатокСтроки) Тогда
        ...
        Иначе
            ...
            Если ПозицияПробела > 0 ИЛИ ПозицияТаба > 0 Тогда
                // Есть что-то после тега, проверяем, что это не комментарий
                НачалоЗначения = 0;
                Если ПозицияПробела > 0 И ПозицияТаба > 0 Тогда
                    НачалоЗначения = Мин(ПозицияПробела, ПозицияТаба);
                ИначеЕсли ПозицияПробела > 0 Тогда
                    НачалоЗначения = ПозицияПробела;
                Иначе
                    НачалоЗначения = ПозицияТаба;
                КонецЕсли;
                
                ЗначениеПослеТега = СокрЛП(Сред(ОстатокСтроки, НачалоЗначения + 1));
                Если НЕ ПустаяСтрока(ЗначениеПослеТега) И НЕ СтрНачинаетсяС(ЗначениеПослеТега, "#") Тогда
                    ЗначениеНаТойЖеСтроке = Истина;
                КонецЕсли;
            КонецЕсли;
        КонецЕсли;
...
Функция НайтиПараметрыМногострочнойСтроки(МассивСтрок)
    ...
    Для Индекс = 0 По МассивСтрок.Количество() - 1 Цикл
        ...
        Если НЕ ПустаяСтрока(ЧастьПослеУправляющейПоследовательности) И НЕ СтрНачинаетсяС(ЧастьПослеУправляющейПоследовательности, "#") Тогда
            Лог.Ошибка("В многострочной строке не должно быть ничего после управляющей последовательности, найдено: " + ЧастьПослеУправляющейПоследовательности);
...
// Разбирает строку с тегом на тег и значение
//
// Параметры:
//   Строка - Строка - строка содержащая тег и значение
//
// Возвращаемое значение:
...
Функция РазобратьТегИЗначение(Строка)
    ...
    Если СтрНачинаетсяС(ОчищеннаяСтрока, "!<") Тогда
        ...
        Если ПозицияЗакрытияТега = 0 Тогда
            ВызватьИсключение "Незакрытый verbatim тег: " + Строка;
        КонецЕсли;
    ...
    Для Инд = 1 По СтрДлина(ОчищеннаяСтрока) Цикл
        ...
        // Ищем пробел или таб, но не в самом начале (не на первой позиции)
    ...
    // Валидация тега согласно YAML спецификации
    // Теги не должны содержать определенные символы, включая {}
    ТегДляВалидации = ТегИЗначение.Тег;
    
    // Проверяем на недопустимые символы в теге
    НедопустимыеСимволы = "{}[],";
    Для Инд = 1 По СтрДлина(НедопустимыеСимволы) Цикл
        Символ = Сред(НедопустимыеСимволы, Инд, 1);
        Если СтрНайти(ТегДляВалидации, Символ) > 0 Тогда
            ВызватьИсключение "Недопустимый символ '" + Символ + "' в теге: " + ТегДляВалидации;
        КонецЕсли;
    КонецЦикла;
...
Функция ПрименитьТегКЗначению(Тег, Значение)
    ...
    ИначеЕсли РазрешенныйТег = "!!map" Тогда
        ...
        ИначеЕсли ТипЗнч(Значение) = Тип("Строка") И СтрНайти(Значение, ":") > 0 Тогда
            // Если это строка с двоеточием (key-value пара), парсим как mapping
            Попытка
                // Парсим как mapping
                МассивСтрок = Новый Массив();
                МассивСтрок.Добавить(Значение);
    ...
    Иначе
        ...
        Если СтрНачинаетсяС(РазрешенныйТег, "!") И НЕ СтрНачинаетсяС(РазрешенныйТег, "!!") И НЕ СтрНачинаетсяС(РазрешенныйТег, "!<") И НЕ СтрНачинаетсяС(РазрешенныйТег, "!tag:") Тогда
            ...
            Если СтрДлина(РазрешенныйТег) > 1 Тогда
                ...
                Исключение
                    // Если произошла ошибка, игнорируем
