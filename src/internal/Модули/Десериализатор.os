...
Функция ЭтоЗначащийСимвол(Строка, Позиция, ИскомыйСимвол)
    ...
    ЭтоJson = ИскомыйСимвол = "{" Или ИскомыйСимвол = "[";
    Если ЭтоJson Тогда
        Строка = СтрЗаменить(Строка, ИскомыйСимвол, "[");
...
Функция ДанныеИзМассиваСтрок(Строки) 
    ...
    ИначеЕсли ТипСодержимого = "Json" Тогда
        ОчищенныеСтроки = ОчиститьСтроки(Строки);
        Возврат ДесериализоватьJSON(СтрСоединить(ОчищенныеСтроки, Символы.ПС));
    Иначе
...
Функция ОпределитьОтступ(Строка)
    ...
    Для а = 1 По СтрДлина(Строка) Цикл
        ...
        //     ВызватьИсключение "Табуляция в отступах не поддерживается. Используйте пробелы.";    
        Иначе
...
Функция ДесериализоватьJSON(СтрокаJSON)
        ...
        Сообщить("Десериализация JSON: " + СтрокаJSON);
