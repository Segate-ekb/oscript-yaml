...
Функция ОбработатьЭкранирующиеПоследовательности(Строка, ТипКавычек)
    ...
    Если ТипКавычек = "double" Тогда
        // Сначала проверяем на недопустимые escape-последовательности
        Если СтрНайти(Результат, "\'") > 0 Тогда
            ВызватьИсключение "Недопустимая escape-последовательность \' в двойных кавычках";
        КонецЕсли;
        
        // Проверяем на недопустимые escape-последовательности
        ПроверитьВалидностьEscapeПоследовательностей(Результат);
        
        // Сначала обрабатываем Unicode и hex escape-последовательности
        Результат = ОбработатьUnicodeEscapeПоследовательности(Результат);
        
        // Заменяем экранированные символы в двойных кавычках
        // Сначала обрабатываем последовательности с реальными символами
        Результат = СтрЗаменить(Результат, "\" + Символы.Таб, Символы.Таб); // Экранированная табуляция (реальный символ)
        
        // Затем обрабатываем текстовые escape последовательности
        Результат = СтрЗаменить(Результат, "\""", """");
        Результат = СтрЗаменить(Результат, "\ ", " ");     // Экранированный пробел
        Результат = СтрЗаменить(Результат, "\n", Символы.ПС);
        ...
        Результат = СтрЗаменить(Результат, "\/", "/");
        Результат = СтрЗаменить(Результат, "\\", "\");     // Двойной backslash в одинарный - должен быть последним
...
Процедура ПроверитьВалидностьEscapeПоследовательностей(Строка)
    ...
    ВалидныеEscapeПоследовательности.Добавить("\N");
    ...
    // Проверяем каждый backslash в строке
    Позиция = 1;
    Пока Позиция <= СтрДлина(Строка) Цикл
        Если Сред(Строка, Позиция, 1) = "\" Тогда
            // Найден backslash, проверяем что за ним следует валидная последовательность
            НайденаВалиднаяПоследовательность = Ложь;
            
            Для Каждого ВалиднаяПоследовательность Из ВалидныеEscapeПоследовательности Цикл
                Если Позиция + СтрДлина(ВалиднаяПоследовательность) - 1 <= СтрДлина(Строка) Тогда
                    Если Сред(Строка, Позиция, СтрДлина(ВалиднаяПоследовательность)) = ВалиднаяПоследовательность Тогда
                        НайденаВалиднаяПоследовательность = Истина;
                        Позиция = Позиция + СтрДлина(ВалиднаяПоследовательность) - 1;
                        Прервать;
                    КонецЕсли;
                КонецЕсли;
            КонецЦикла;
            
            // Проверяем Unicode escape последовательности \uXXXX и \UXXXXXXXX
            Если НЕ НайденаВалиднаяПоследовательность Тогда
                Если Позиция + 1 <= СтрДлина(Строка) И Сред(Строка, Позиция + 1, 1) = "u" Тогда
                    // Проверяем \uXXXX (4 hex цифры)
                    Если Позиция + 5 <= СтрДлина(Строка) Тогда
                        HexЧасть = Сред(Строка, Позиция + 2, 4);
                        Если ПроверитьHexСтроку(HexЧасть) Тогда
                            НайденаВалиднаяПоследовательность = Истина;
                            Позиция = Позиция + 5;
                        КонецЕсли;
                    КонецЕсли;
                ИначеЕсли Позиция + 1 <= СтрДлина(Строка) И Сред(Строка, Позиция + 1, 1) = "U" Тогда
                    // Проверяем \UXXXXXXXX (8 hex цифр)
                    Если Позиция + 9 <= СтрДлина(Строка) Тогда
                        HexЧасть = Сред(Строка, Позиция + 2, 8);
                        Если ПроверитьHexСтроку(HexЧасть) Тогда
                            НайденаВалиднаяПоследовательность = Истина;
                            Позиция = Позиция + 8;
                        КонецЕсли;
                    КонецЕсли;
                ИначеЕсли Позиция + 1 <= СтрДлина(Строка) И Сред(Строка, Позиция + 1, 1) = "x" Тогда
                    // Проверяем \xXX (2 hex цифры)
                    Если Позиция + 3 <= СтрДлина(Строка) Тогда
                        HexЧасть = Сред(Строка, Позиция + 2, 2);
                        Если ПроверитьHexСтроку(HexЧасть) Тогда
                            НайденаВалиднаяПоследовательность = Истина;
                            Позиция = Позиция + 3;
                        КонецЕсли;
...
Функция ОбработатьUnicodeEscapeПоследовательности(Строка)
    ...
    Пока Позиция <= СтрДлина(Результат) - 5 Цикл
        Если Сред(Результат, Позиция, 2) = "\u" Тогда
            HexЧасть = Сред(Результат, Позиция + 2, 4);
            Если ПроверитьHexСтроку(HexЧасть) Тогда
                // Преобразуем hex в число и затем в символ
                Попытка
                    КодСимвола = ПреобразоватьHexВЧисло(HexЧасть);
                    НовыйСимвол = Символ(КодСимвола);
                    Результат = Лев(Результат, Позиция - 1) + НовыйСимвол + Сред(Результат, Позиция + 6);
                    // Корректируем позицию после замены
                    Позиция = Позиция + 1;
                Исключение
    ...
    Пока Позиция <= СтрДлина(Результат) - 3 Цикл
        Если Сред(Результат, Позиция, 2) = "\x" Тогда
            HexЧасть = Сред(Результат, Позиция + 2, 2);
            Если ПроверитьHexСтроку(HexЧасть) Тогда
                // Преобразуем hex в число и затем в символ
                Попытка
                    КодСимвола = ПреобразоватьHexВЧисло(HexЧасть);
                    НовыйСимвол = Символ(КодСимвола);
                    Результат = Лев(Результат, Позиция - 1) + НовыйСимвол + Сред(Результат, Позиция + 4);
                    // Корректируем позицию после замены
                    Позиция = Позиция + 1;
                Исключение
                    // Если не удалось преобразовать, пропускаем
...
Функция НайтиТипДанныхПоСтроке(Знач Строка)
    ...
    // Специальная проверка на неправильный синтаксис многострочных строк
    ...
    // Сначала проверяем управляющие символы структуры (двоеточие, тире и т.д.)
    // Это важнее чем проверка на кавычки
    Если ДанныеУправляющегоСимвола.Символ <> "" Тогда
        Возврат ОпределитьТипПоУправляющемуСимволу(ДанныеУправляющегоСимвола, Строка);
    КонецЕсли;
    
    // Специальная проверка на строки в кавычках - они скаляры, но только если нет структурных символов
    // Проверяем, есть ли в строке кавычки, которые могут указывать на многострочную строку
    Если СтрНайти(ОчищеннаяСтрока, """") > 0 ИЛИ СтрНайти(ОчищеннаяСтрока, "'") > 0 Тогда
        // Проверим, это однострочная закрытая кавычка или многострочная
        Если СтрНачинаетсяС(ОчищеннаяСтрока, """") ИЛИ СтрНачинаетсяС(ОчищеннаяСтрока, "'") Тогда
            Лог.Отладка("Найдена строка в кавычках, обрабатываем как скаляр: " + ОчищеннаяСтрока);
            Возврат "Скаляр";
        Иначе
            // Кавычка есть, но не в начале - возможно это многострочная строка
            // Нужно проверить более внимательно
            ПозицияПервойКавычки = Макс(СтрНайти(ОчищеннаяСтрока, """"), СтрНайти(ОчищеннаяСтрока, "'"));
            Если ПозицияПервойКавычки > 0 Тогда
                // Получаем символ кавычки
                ТипКавычки = Сред(ОчищеннаяСтрока, ПозицияПервойКавычки, 1);
                // Проверяем, закрывается ли кавычка в этой же строке
                ...
                Если ПозицияПоследнейКавычки > ПозицияПервойКавычки Тогда
                    // Кавычки закрываются в той же строке - это обычный скаляр
                ...
                ИначеЕсли ПозицияПоследнейКавычки = ПозицияПервойКавычки Тогда
                    // Кавычка не закрывается - возможно многострочная строка
                    Лог.Отладка("Найдена незакрытая строка в кавычках, обрабатываем как скаляр: " + ОчищеннаяСтрока);
                    Возврат "Скаляр";
                Иначе
                    // Другие случаи с кавычками - оставляем как есть для дальнейшей обработки
    ...
    // Если ничего особенного не найдено - это скаляр
    Лог.Отладка("Управляющий символ не найден, определен как скаляр");
    Возврат "Скаляр";
...
Функция ОбработатьСкалярноеЗначение(Строки)
    ...
    // Проверяем на недопустимые комментарии в plain scalar ПЕРЕД очисткой строк
    ПерваяСтрока = "";
    Если Строки.Количество() > 0 Тогда
        ПерваяСтрока = СокрЛП(Строки[0]);
    КонецЕсли;
    
    // Если это не quoted строка и не блочный скаляр и есть несколько строк, проверяем на комментарии внутри значения
    Если НЕ СтрНачинаетсяС(ПерваяСтрока, """") И НЕ СтрНачинаетсяС(ПерваяСтрока, "'") И НЕ СтрНачинаетсяС(ПерваяСтрока, "|") И НЕ СтрНачинаетсяС(ПерваяСтрока, ">") И Строки.Количество() > 1 Тогда
        // Проверяем, действительно ли это многострочный plain scalar
        // Считаем количество непустых строк (не являющихся комментариями)
        ...
        // Только если у нас больше одной значимой строки, проверяем на комментарии внутри
        Если КоличествоЗначимыхСтрок > 1 Тогда
            Для Индекс = 0 По Строки.ВГраница() Цикл
                СтрокаДляПроверки = СокрЛП(Строки[Индекс]);
                Если НЕ ЭтоПустаяСтрокаПослеОбрезки(СтрокаДляПроверки) И НЕ СтрНачинаетсяС(СтрокаДляПроверки, "#") Тогда
                    // Это значимая строка, проверяем есть ли в ней комментарий
            ...
            КонецЦикла;
        КонецЕсли;
        // Ищем первую значимую строку (не комментарий и не пустую)
        ИндексПервойЗначимойСтроки = -1;
        Для Индекс = 0 По Строки.ВГраница() Цикл
            ОбрезаннаяСтрока = СокрЛП(Строки[Индекс]);
            Если НЕ ЭтоПустаяСтрокаПослеОбрезки(Строки[Индекс]) И Лев(ОбрезаннаяСтрока, 1) <> "#" Тогда
                ИндексПервойЗначимойСтроки = Индекс;
                Прервать;
            КонецЕсли;
        КонецЦикла;
        
        // Если нашли значимую строку, проверяем что после нее нет комментариев внутри значения
        Если ИндексПервойЗначимойСтроки >= 0 Тогда
            ВстретилиКомментарий = Ложь;
            Для Индекс = ИндексПервойЗначимойСтроки + 1 По Строки.ВГраница() Цикл
                ОбрезаннаяСтрока = СокрЛП(Строки[Индекс]);
                Если НЕ ЭтоПустаяСтрокаПослеОбрезки(Строки[Индекс]) Тогда
                    ...
                    ИначеЕсли ВстретилиКомментарий Тогда
                        // После комментария идет еще одна значимая строка - это ошибка
                        ВызватьИсключение "Комментарий недопустим внутри plain scalar значения";
                    КонецЕсли;
                КонецЕсли;
            КонецЦикла;
    ...
    // Проверяем тип кавычек до объединения строк
    ПерваяСтрока = "";
    Если ОчищенныеСтроки.Количество() > 0 Тогда
        ПерваяСтрока = ОчищенныеСтроки[0];
    КонецЕсли;
    
    // Если это double-quoted строка, обрабатываем по-особому
    Если СтрНачинаетсяС(ПерваяСтрока, """") Тогда
        ИтоговаяСтрока = СобратьDoubleQuotedСтрокуИзМассива(ОчищенныеСтроки);
    Иначе
...
Процедура ПроверитьИсходныеСтрокиНаВложенныеЯкори(МассивСтрок, ПараметрыЯкоря)
    ...
    Если НЕ ПустаяСтрока(СодержимоеПослеЯкоря) И СтрНачинаетсяС(СодержимоеПослеЯкоря, "&") И СтрНайти(СодержимоеПослеЯкоря, ":") = 0 Тогда
        // Это попытка создать якорь для простого скаляра, который уже имеет якорь
        Лог.Ошибка("Узел не может иметь несколько якорей");
        ВызватьИсключение "Узел не может иметь несколько якорей";
    КонецЕсли;
    
    // Проверяем строки содержимого на наличие дополнительных якорей для скалярных значений
    ...
    Для Индекс = ПараметрыЯкоря.ИндексСтрокиСЯкорем + 1 По МассивСтрок.ВГраница() Цикл
        ...
        УровеньОтступаСтроки = ОпределитьОтступ(СтрокаСодержимого);
        Если УровеньОтступаСтроки <= УровеньОтступаЯкоря И НЕ ПустаяСтрока(СокрЛП(СтрокаСодержимого)) Тогда
            Прервать;
        КонецЕсли;
...
Функция ИзвлечьСодержимоеМногострочнойСтроки(МассивСтрок, Параметры)
    ...
    Для Индекс = Параметры.ИндексНачалаСодержимого По МассивСтрок.Количество() - 1 Цикл
        ...
        ОтступТекущейСтроки = ОпределитьОтступ(ИсходнаяСтрока);
        
        Если ОтступТекущейСтроки >= Параметры.ОтступСодержимого Тогда
            СтрокаБезОтступа = Сред(ИсходнаяСтрока, Параметры.ОтступСодержимого + 1);
        Иначе
            // Если отступ меньше базового и это пустая строка (только пробелы), делаем ее пустой
            Если ЭтоПустаяСтрокаПослеОбрезки(ИсходнаяСтрока) Тогда
                СтрокаБезОтступа = "";
            Иначе
                // Если отступ меньше базового, но строка не пустая - оставляем как есть
                СтрокаБезОтступа = ИсходнаяСтрока;
            КонецЕсли;
        КонецЕсли;
        
        // Проверяем, является ли строка комментарием после удаления отступа
        // Комментарии удаляются только если их отступ меньше отступа содержимого
        СтрокаБезОтступаОбрезанная = СокрЛП(СтрокаБезОтступа);
        Если СтрНачинаетсяС(СтрокаБезОтступаОбрезанная, "#") Тогда
            ОтступСтрокиКомментария = ОпределитьОтступ(ИсходнаяСтрока);
            Если ОтступСтрокиКомментария < Параметры.ОтступСодержимого Тогда
                Продолжить; // Пропускаем комментарии с меньшим отступом
            КонецЕсли;
            // Комментарии с отступом >= отступа содержимого считаются частью содержимого
        КонецЕсли;
...
Функция СобратьМногострочнуюСтроку(СтрокиСодержимого, Параметры)
    ...
    Иначе
        // Keep chomping (+): оставляем все как есть, ничего не делаем - строка уже собрана правильно
    КонецЕсли;
...
Функция СобратьDoubleQuotedСтрокуИзМассива(МассивСтрок)
    ...
    Для Индекс = 0 По МассивСтрок.ВГраница() Цикл
        ...
        // Если это пустая строка, то это должен быть перенос строки
        Если ЭтоПустаяСтрокаПослеОбрезки(Строка) Тогда
        ...
        // Если строка заканчивается на \ (не экранированный), то это продолжение на следующей строке
        Если СтрЗаканчиваетсяНа(Строка, "\") И НЕ СтрЗаканчиваетсяНа(Строка, "\\") Тогда
            // Убираем \ в конце
            ...
            Если Индекс > 0 И НЕ БылЭкранированныйПеренос И НЕ БылПереносСтроки Тогда
                Результат = Результат + " ";
            КонецЕсли;
            
            Результат = Результат + СтрокаБезBackslash;
...
//
Функция СоздатьПараметрыЯкоряСОтступом(ИмяЯкоря, ИндексСтрокиСЯкорем, УровеньОтступа)
    Параметры = Новый Структура();
...
// Создает структуру параметров якоря (устаревшая версия)
//
// Параметры:
//   ИмяЯкоря - Строка - имя якоря
//   ИндексСтрокиСЯкорем - Число - индекс строки с якорем
...
Функция СоздатьПараметрыЯкоря(ИмяЯкоря, ИндексСтрокиСЯкорем)
    ...
    Параметры.Вставить("ИмяЯкоря", ИмяЯкоря);
    Параметры.Вставить("ИндексСтрокиСЯкорем", ИндексСтрокиСЯкорем);
    Параметры.Вставить("УровеньОтступа", 0); // По умолчанию отступ 0
...
Функция НайтиПараметрыЯкоря(МассивСтрок)
    ...
    Для Индекс = 0 По МассивСтрок.Количество() - 1 Цикл
        ...
        Параметры = СоздатьПараметрыЯкоряСОтступом(ИмяЯкоря, Индекс, УровеньОтступа);
