...
Функция ЭтоМногострочныйЛитерал(ЗначениеСтрока)
	// Проверяем, является ли значение многострочным литералом
	// Literal style: | (сохраняет переводы строк)
	// Folded style: > (сворачивает переводы строк в пробелы)
...
Функция СчитатьМногострочныйБлок(Строки, Индекс, ТекущийУровеньОтступа, ТипЛитерала)
	...
	БазовыйОтступ = -1; // Базовый отступ содержимого (в символах)
	
	// Сначала найдем первую непустую строку для определения базового отступа
	Пока ИндексСтроки <= Строки.ВГраница() Цикл
		...
		// Определяем базовый отступ в символах
		БазовыйОтступ = ПолучитьОтступВСимволах(Строка);
		Прервать;
	КонецЦикла;
	
	// Теперь обрабатываем все строки
	ИндексСтроки = Индекс + 1;
	Пока ИндексСтроки <= Строки.ВГраница() Цикл
		Строка = Строки[ИндексСтроки];
	...
	// Убираем завершающие пробелы и пустые строки
	МногострочноеЗначение = ОчиститьЗавершающиеСимволы(МногострочноеЗначение, ТипЛитерала);
...
Функция ПарсингFlowStyleМассив(ЗначениеСтрока)
	...
	Для Каждого Элемент Из Элементы Цикл
		...
		// Проверяем специально на null прежде чем на другие структуры
		Если НРег(ЗначениеЭлемента) = "null" Тогда
			МассивРезультат.Добавить(Неопределено);
		// Проверяем, является ли элемент вложенной структурой (Flow style)
		ИначеЕсли Лев(ЗначениеЭлемента, 1) = "[" И Прав(ЗначениеЭлемента, 1) = "]" ИЛИ 
				Лев(ЗначениеЭлемента, 1) = "{" И Прав(ЗначениеЭлемента, 1) = "}" Тогда
			ВложеннаяСтруктура = ПарсингFlowStyle(ЗначениеЭлемента);
			МассивРезультат.Добавить(ВложеннаяСтруктура);
		Иначе
			МассивРезультат.Добавить(ПреобразоватьЗначение(ЗначениеЭлемента));
		КонецЕсли;
	КонецЦикла;
...
Функция ПарсингFlowStyleОбъект(ЗначениеСтрока)
	...
	Для Каждого Пара Из Пары Цикл
		...
		Если ПозицияДвоеточия = 0 Тогда
			Продолжить; // Нет двоеточия, некорректная пара
		КонецЕсли;
		...
		// Проверяем, является ли значение вложенной структурой (Flow style)
