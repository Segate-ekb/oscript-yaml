...
Процедура СоздатьТестовыеСценарииПоТегам()
	ПутьКТегу = ОбъединитьПути(ПутьККаталогуТестов(), "tags");

	Логгер.Информация("Создание тестовых сценариев по тегам в каталоге: %1", ПутьКТегу);
	Если Не ФС.КаталогСуществует(ПутьКТегу) Тогда
		Логгер.Ошибка("Каталог тегов <%1> не найден! Пропускаем создание сценариев", ПутьКТегу);
		Возврат;
	КонецЕсли;

	ФайлыТегов = НайтиФайлы(ПутьКТегу, "*", Ложь);
	ИмяШаблона = ОбъединитьПути(ТекущийСценарий().Каталог, "test-template");
	Если Не ФС.ФайлСуществует(ИмяШаблона) Тогда
		Логгер.Ошибка("Шаблон тестового сценария <%1> не найден! Пропускаем создание сценариев", ИмяШаблона);
		Возврат;
	КонецЕсли;

	ТекстТестовогоНабора = ПолучитьЛистингШаблонаТестовогоНабора(ИмяШаблона);

	Для каждого Каталог Из ФайлыТегов Цикл
		Логгер.Отладка("Обработка каталога тега: %1", Каталог.ПолноеИмя);
		Если Не Каталог.ЭтоКаталог() Тогда
			Логгер.Отладка("Пропускаем файл <%1>, так как это не каталог", Каталог.ПолноеИмя);
			Продолжить;
		КонецЕсли;

		ИмяТега = Каталог.Имя;
		//Нормализуем имя файла
		ИмяФайла = СтрЗаменить(ИмяТега, ".", "_") + ".os";
		ИмяФайла = СтрЗаменить(ИмяФайла, "-", "_");

		ПутьКтестовомуСценарию = ОбъединитьПути("tests", ИмяФайла);

		Логгер.Отладка("Путь к тестовому сценарию: %1", ПутьКтестовомуСценарию);

		Если ФС.КаталогСуществует(ПутьКтестовомуСценарию) Тогда
			Логгер.Отладка("Тестовый сценарий <%1> найден! Пропускаем создание сценария", ПутьКтестовомуСценарию);
			Продолжить;
		КонецЕсли;
		ТекстФайла = СтрЗаменить(ТекстТестовогоНабора, "%ИМЯ_ТЕГА%", ИмяТега);

		ЗаписатьТестовыйНабор(ТекстФайла, ПутьКтестовомуСценарию);
	КонецЦикла;

КонецПроцедуры

Функция ПолучитьЛистингШаблонаТестовогоНабора(ИмяШаблона)


	ЧтениеТекста = Новый ЧтениеТекста(ИмяШаблона, "UTF-8");
	ТекстТестовогоНабора = ЧтениеТекста.Прочитать();
	ЧтениеТекста.Закрыть();

	Логгер.Отладка("Получен текст шаблона тестового набора: %1", ИмяШаблона);

	Возврат ТекстТестовогоНабора;
