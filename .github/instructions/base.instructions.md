...
## Основная информация
Этот файл содержит инструкции по разработке и форматированию кода для проекта. Пожалуйста, следуйте этим рекомендациям, чтобы обеспечить единообразие и качество кода.
Данный код разрабатывается на языке OneScript
- Перед написанием какого-либо кода, необходимо написать тест, который этот код проверит.
- Перед коммитом необходимо убедиться, что все тесты проходят успешно.
- Используй принципы DRY и SOLID  при разработке.
- Каждый метод должен обладать единой ответственностью (Single Responsibility Principle).
- Тесты должны распологаться в папке `tests` в корне проекта.
