...
Процедура ТестНекорректногоУровняОтступа() Экспорт
    ...
    ПервыйУровень = Результат.Получить("first_level");
    Ожидаем.Что(ПервыйУровень).Не_().ЭтоНеопределено();
    Ожидаем.Что(ПервыйУровень.Получить("second_level_a")).Равно("value1");
    
    // Проверка обработки некорректного отступа - должен быть на уровне first_level
    Ожидаем.Что(Результат.Получить("third_level_incorrect")).Не_().ЭтоНеопределено();
КонецПроцедуры
